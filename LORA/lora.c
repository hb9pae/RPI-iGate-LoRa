...
uint32_t  freq = 433775000 ; // in Mhz! (868.1)
...
//	writeReg(REG_SYNC_WORD, 0x34); // LoRaWAN public sync word
	writeReg(REG_SYNC_WORD, 0x12); // LoRaWAN public sync word
...
// see comment hb9pae		rssicorr = 157;
				rssicorr = 137;
			}
...
void showreg() {
		...
		byte value = readReg(REG_VERSION);
		printf("Version: 0x%x\n", value);

		value = readReg(REG_FRF_MSB);
		printf("FRF_MSB: 0x%x\n", value);
		value = readReg(REG_FRF_MID);
		printf("FRF_MID: 0x%x\n", value);
		value = readReg(REG_FRF_LSB);
		printf("FRF_LSB: 0x%x\n", value);

		value = readReg(REG_SYNC_WORD);
		printf("Sync Word: 0x%x\n", value);

		value = readReg(REG_FIFO);
		printf("FIFO: 0x%x\n", value);
		value = readReg(REG_OPMODE);
		printf("OPMODE: 0x%x\n", value);
		value = readReg(REG_FIFO_ADDR_PTR);
		printf("FIFO Addr: 0x%x\n", value);
		value = readReg(REG_FIFO_TX_BASE_AD);
		printf("FIFO TX Base: 0x%x\n", value);
		value = readReg(REG_FIFO_RX_BASE_AD);
		printf("FIFO RX Base: 0x%x\n", value);
		value = readReg(REG_RX_NB_BYTES);
		printf("RX NB Bytes: 0x%x\n", value);
		value = readReg(REG_FIFO_RX_CURRENT_ADDR);
		printf("FIFO RX Current: 0x%x\n", value);
		value = readReg(REG_IRQ_FLAGS);
		printf("IRQ: 0x%x\n", value);
		value = readReg(REG_DIO_MAPPING_1);
		printf("DIO 1: 0x%x\n", value);
		value = readReg(REG_DIO_MAPPING_2);
		printf("DIO 2: 0x%x\n", value);
		value = readReg(REG_MODEM_CONFIG);
		printf("Modem Config: 0x%x\n", value);
		value = readReg(REG_MODEM_CONFIG2);
		printf("Modem Config 2: 0x%x\n", value);
		value = readReg(REG_MODEM_CONFIG3);
		printf("Modem Config 3: 0x%x\n", value);
		value = readReg(REG_SYMB_TIMEOUT_LSB);
		printf("SYMB Timeout: 0x%x\n", value);
		value = readReg(REG_PKT_SNR_VALUE);
		printf("SNR: 0x%x\n", value);
		value = readReg(REG_PAYLOAD_LENGTH);
		printf("PayLoad: 0x%x\n", value);
		value = readReg(REG_IRQ_FLAGS_MASK);
		printf("IRW: 0x%x\n", value);
		value = readReg(REG_MAX_PAYLOAD_LENGTH);
		printf("PayLoad LNG: 0x%x\n", value);
		value = readReg(REG_HOP_PERIOD);
		printf("Hop Period: 0x%x\n", value);
		value = readReg(REG_SYNC_WORD);
		printf("Sync Word: 0x%x\n", value);
		value = readReg(REG_VERSION); 
		printf("Version: 0x%x\n", value);
}
...
int main (int argc, char *argv[]) {
	if (argc < 2) {
		printf ("Usage: argv[0] led|test|sender|rec [message]\n");
		exit(1);
	}
	
	wiringPiSetup () ;   //Use WiringPi pin number
